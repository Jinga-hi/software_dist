...
PASSIVE_TIMEOUT=10.0  # Quit if no packets received for this many seconds.
if os.name == 'nt':
...
class Capture():
  ...
  def gen_filename(self):
    if self.windows:
      return os.path.join(DATA_PATH, datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d_%H-%M-%S_') + self.fileroot + '.mat')
    else:
    ...
    while self.keep_capturing:
      ...
      try:
        ...
        if not self.timeout_set:
          ...
          self.s.settimeout(PASSIVE_TIMEOUT)
